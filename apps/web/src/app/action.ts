...
export async function getPeer(peerId: string): Promise<Peer> {
  await requireAdmin();
  const result = await apiRequest<{ peer: Peer }>(
...
export async function createPeer(): Promise<CreatedPeer> {
  ...
  const result = await signedApiRequest<CreatedPeer & { clientConfig: string }>(
    "/peers",
    "POST",
    {
  ...
  return {
    ...result,
    clientConfig: result.clientConfig.replace(
      "<CLIENT_PRIVATE_KEY>",
...
export async function setTunnelState(
  state: "up" | "down",
...
export async function revokePeer(peerId: string): Promise<Peer> {
  const result = await signedApiRequest<{ peer: Peer }>(
    `/peers/${encodeURIComponent(peerId)}`,
    "DELETE",
    { peerId },
  );
