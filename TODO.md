- create Debian package that installs and uninstalls cleanly [ ]
  - write electron-builder configs that package the app, native service, configs and required files [x]
  - create install/uninstall scripts that place files in the correct system paths [ ]
  - create polkit policy [ ]
  - write native service (Rust/C++) that manages low-level VPN connection and config operations [ ]
  - write systemd service that runs the native service [ ]
  - write D-Bus policy/config [ ]
